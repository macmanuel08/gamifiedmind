...
class QuizModel {
    ...
    displayAnswers() {
        ...
        questionBlocks.forEach((questionBlock, i) => {
            const selectedOption = questionBlock.querySelector('.option.selected');
            let selectedOptionValue;

            selectedOption == null ? selectedOptionValue = null : selectedOptionValue = selectedOption.dataset.value;

            if ( selectedOptionValue != answers[i] || selectedOptionValue == null) {
                ...
                questionBlock.appendChild(explanationParagraph);
            } else {
    ...
    displayScore() {
        ...
        scoreContainer.textContent = `You got: ${this.getScore()} points. ${this.getScore() <= 3 ? 'Nice Try.' : 'Awesome!'}`;
